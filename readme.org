- // NOTE with the dev tools setup, currently using old svelte-mode and it seems to switch on type-script mode... not sure why, should give ts another go.
...
- [ ] refactor
  - LEFTOFF - leftoff on going through each file as high level, got to article (added jquery), haven't touched the other compoonents yet
