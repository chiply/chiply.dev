- [-] pre publication checklist
  ...
  - [ ] pull routes out of about
  - [ ] make title work (eg whatever is displayed in post.org) (simply add title back in, that's sufficient for now)
    

  - [ ] sample articles
    - [ ] polish the sample articles
    - [ ] fix homepage -- page1 links to post0
    - [ ] .org for index.html, should be another article
      - [ ] automatically build data list from this
    
  - [ ] double check mobile experience and add necessary compoonents
  - [ ] final styling
    - [ ] font size
    - [ ] padding
    - [ ] all hover
...
- [X] get env set up for svelte stuff (lsp + webmode etc...)
  - navigation
  - find refs
  - autocompletion for JS stuff
  - autocompletion for tags
  - emmet?
  - current state -- using a weird mashup of svelte-mode (not svlete-ts-mode as i couldn't gett the grgammar insttalled).  note you'll see typescript-mode when the buffers are activated, but this is actutally sveltte-mode defined in ttypsecript mode? (see thte C-h m)... but this works well enough for now...
    
- [X] initial refactor including jquery
  - [X] isWide over isMobile and make this state
  - [X] remove unnecessary onMount lifecyle hooks.  understand exactly what is happening when they're used together: A: i still dono't fully understand but my issue is relatetd to my use case.  i'm calling onmount inside effect which is an antipattern, but it's the only way i can get it to work
